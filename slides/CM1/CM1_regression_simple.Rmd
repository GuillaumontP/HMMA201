...
segments(x0 = TV, y0 = sales, x1 = TV, y1 = predict(ad.fit))
```
$$\hat{\sigma}^2 = \frac{1}{n-2} \cdot RSS = \frac{1}{n-2} \sum_{i = 1}^n (y_i - \hat{\beta}_0 - \hat{\beta}_1 x_i)^2 = `r round(sum((sales - predict(ad.fit))^2) / (n-2), 2)`$$
...
&= \|\hat{\mathbf{y}} - \bar{y} \mathbb{1} + \hat{\epsilon}\|^2 \\
&= \|\hat{\mathbf{y}} - \bar{y} \mathbb{1}\|^2  + \|\hat{\epsilon}\|^2 
\end{aligned}
$$
...
```
$$
\hat{\sigma}^2 = \frac{1}{n-2} \cdot RSS = `r round(sum((sales - predict(ad.fit))^2) / (n-2), 2)`
\quad
